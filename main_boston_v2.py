...
diabetes_train = r'boston_data_train.csv'
df_train = pd.DataFrame(data=x_train)
df_train[8] = y_train 
df_train.to_csv(diabetes_train, index = False, header = False)

# write out test data
diabetes_test = r'boston_data_test.csv'
...
mse_file_name = r'mse_boston.csv'
...
if __name__ == '__main__':
    ...
    data_logz = pd.read_csv("logz_boston.csv", header = None) 
    ...
    data_mse = pd.read_csv("mse_boston.csv", header = None) 
    mse_test_avarage = data_mse.mean()
    
    with open(r'mse_boston_mean.csv', 'a') as f:
        writer = csv.writer(f)
    ...
    plt.show()
    fig.savefig('boston_log_evidence.png')
