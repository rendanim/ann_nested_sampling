...
def calc_evidence_and_mse(i):
    ...
    nlive = 100 * ndim_1
    logl_args = [input_neurons, hidden_neurons, output_neurons, beta[i-1][0]]
    ...
    sampler.run_nested(print_progress = False, maxiter = ITERATIONS, maxbatch = 0, nlive_init = nlive)
    res = sampler.results
...
if __name__ == '__main__':
    ...
    data_logz = pd.read_csv("logz_yacht.csv", header = None) 
    ...
    data_mse = pd.read_csv("mse_yacht.csv", header = None) 
    mse_test_avarage = data_mse.mean()
    
    with open(r'mse_yacht_mean.csv', 'a') as f:
        writer = csv.writer(f)
    ...
    plt.show()
    fig.savefig('yacht_log_evidence.png')
